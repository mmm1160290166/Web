    ...
    $.fn.bcalendar = function (option) {
        ...
        function populate() {
            ...
            if (option.url && option.url != "") {
                ...
                console.log(dateFormat.call(option.showday, i18n.xgcalendar.dateformat.fulldayvalue));
                if (option.extParam) {
                ...
                $.ajax({
                    type: "POST",
                    url: '/ajax/post',
                    headers: { "action": option.url, "usercode": Session.UserCode },
                    data: { "CREATEUSER": Session.UserCode, "NowDate": dateFormat.call(option.showday, i18n.xgcalendar.dateformat.fulldayvalue), "ViewType": option.view, "Zone": zone },
                    async: true,
                    dataType: "json",
                            ...
                            if (d.DATA.length > 0) {
                                for (var i = 0; i < d.DATA.length; i++) {
                                    var event = [
                                        ...
                                        d.DATA[i].GUID,
                                        d.DATA[i].Day01,
                                        strtodate(d.DATA[i].Day02),
                                        strtodate(d.DATA[i].Day03),
                                        d.DATA[i].Day04,
                                        d.DATA[i].Day05,
                                        d.DATA[i].Day06,
                                        //d.DATA[i].Day07,
                                    ...
                                    var Jsond = { issort: true, events: event, start: strtodate(d.DATA[i].Day02), end: strtodate(d.DATA[i].Day03) };
                                    //★1
                                    responseData(Jsond, Jsond.start, Jsond.end);
                                    pushER(Jsond.start, Jsond.end);
                                }
                            }
                            else {
                                responseData([], new Date(),new Date());
                                pushER(new Date(),new Date());
                            }
                        }
        ...
        function quickd(type) {
            ...
            $.ajax({
                type: "POST",
                url: "/ajax/Post",
                data: { "CREATEUSER": Session.UserCode, "Guid": calid },
                headers: { action: option.quickDeleteUrl, usercode: Session.UserCode },
                //async: true,
                async: true,
                dataType: "json",
                success: function (d) {
                    if (d.RST == 'SUC') {
                        ...
                        top.layer.msg('删除失败!', { icon: 2, time: 2000 }, function () {
            ...
            if (option.quickUpdateUrl != "" && data[8] == 1 && option.readonly != true) {
                ...
                if (option.quickUpdateHandler && $.isFunction(option.quickUpdateHandler)) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/ajax/Post",
                        data: { "CREATEUSER": Session.UserCode, "Guid": id, "Day02": dateFormat.call(start, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm"), "Day03": dateFormat.call(end, i18n.xgcalendar.dateformat.fulldayvalue + " HH:mm"),"Zone":zone },
                        headers: { action: option.quickUpdateUrl, usercode: Session.UserCode },
                        //async: true,
                        async: true,
                        dataType: "json",
                        success: function (d) {
                            if (d.RST == 'SUC') {
                                ...
                                //top.layer.msg('更新成功!', { icon: 1, time: 2000 }, function () {
                                //}); 
                            }
                            else {
                                top.layer.msg('更新失败!', { icon: 2, time: 2000 }, function () {
        ...
        function quickadd(start, end, isallday, pos) {
            ...
            if (buddle.length == 0) {
                ...
                temparr.push('</th><td class="cb-value"><div class="textbox-fill-wrapper"><div class="textbox-fill-mid"><input id="bbit-cal-what" class="textbox-fill-input" placeholder="标题..."/><input type="text" id="bbit-cal-didian" class="textbox-fill-input" placeholder="地点..."/><input type="text" id="bbit-cal-person" class="textbox-fill-input" placeholder="参与人..."/></div></div><div class="cb-example">');
                //<select id="bbit-cal-isedit"><option value="1" selected="selected">允许</option><option value="-1">不允许</option></select>
                ...
                calbutton.click(function (e) {
                    ...
                    var didian = $("#bbit-cal-didian").val();
                    var person = $("#bbit-cal-person").val();
                    var datestart = $("#bbit-cal-start").val();
                    var dateend = $("#bbit-cal-end").val();
                    var allday = $("#bbit-cal-allday").val();
                    //var isedit = $("#bbit-cal-isedit").val();
                    var isedit = '1';
                    var zone = new Date().getTimezoneOffset() / 60 * -1;
                    var f = /^[^\$\<\>]+$/.test(what);
                    var f1 = /^[^\$\<\>]+$/.test(didian);
                    var f2 = /^[^\$\<\>]+$/.test(person);
                    if (!f) {
                        top.layer.msg(i18n.xgcalendar.invalid_title, { icon: 2, time: 2000 }, function () { });
                        $("#bbit-cal-what").focus();
                        option.isloading = false;
                        return false;
                    }
                    if (!f1) {
                        top.layer.msg(i18n.xgcalendar.invalid_title.replace('标题', '地点'), { icon: 2, time: 2000 }, function () { });
                        $("#bbit-cal-didian").focus();
                        option.isloading = false;
                        return false;
                    }
                    if (!f2) {
                        top.layer.msg(i18n.xgcalendar.invalid_title.replace('标题', '参与人'), { icon: 2, time: 2000 }, function () { });
                        $("#bbit-cal-person").focus();
                        option.isloading = false;
                        return false;
                    }
                    var param = {
                        CREATEUSER: Session.UserCode,
                        ViewType: option.view,//类型
                        Day01: what,//标题
                        Day02: datestart,//开始时间
                        Day03: dateend,//结束时间
                        Day04: allday,//是否全天事件
                        Day05: DateDiff("d", strtodate(datestart), strtodate(dateend)) > 1 ? 1 : 0,//是否跨日日程
                        Day06: '0',//是否循环日程
                        Day07: '1',//主题
                        Day08: isedit,//是否允许修改
                        Day09: didian,//地点
                    ...
                    if (option.quickAddHandler && $.isFunction(option.quickAddHandler)) {
                        ...
                        var diff = DateDiff("d", sd, ed);
                        newdata.push(sd, ed, allday == "1" ? 1 : 0, diff > 0 ? 1 : 0, 0);
                        newdata.push(param.Day07, param.Day08, param.Day09, param.Day10);
                        ...
                        $.ajax({
                            ...
                            url: "/ajax/Post",
                            data: param,
                            headers: { action: option.quickAddUrl, usercode: Session.UserCode },
                            //async: true,
                            async: true,
                            dataType: "json",
                            success: function (d) {
                                if (d.RST == 'SUC') {
                                    //★添加数据
                                    option.isloading = false;
                                    option.eventItems[tId][0] = d.DATA;
                                    option.eventItems[tId][8] = 1;
                                    render();
                                    option.onAfterRequestData && option.onAfterRequestData(2);
                                }
                                else {
                                    option.onRequestDataError && option.onRequestDataError(2, d);
                                    option.isloading = false;
                                    option.onAfterRequestData && option.onAfterRequestData(2);
                                }
                            },
                    ...
                    //layer.open({
                    //    type: 2,
                    //    title: ['参与人选择', 'height:30px;color:#fff;font-size:14px; background: rgb(79,129,189); border-bottom: 1px solid rgb(188,188,188);line-height:30px;font-family:"Microsoft YaHei";'],
                    //    shade: [0.3, '#393D49'],
                    //    closeBtn: 0,
                    //    area: ['300px', '85%'],
                    //    content: '/pop/DeptUserMultiple?key=bbit-cal-personc&title=bbit-cal-person&type=1',
                    //    end: function () {
                    //        if ($("#bbit-cal-personc").val() == '') {
                    //            param =
                    //                {
                    //                    CREATEUSER: Session.UserCode,
                    //                    ViewType: option.view,//类型
                    ...
                    //                Day07: '1',//主题
                    //                Day08: $("#bbit-cal-didian").val(),//地点
                    //                Day09: $("#bbit-cal-person").val(),//参与人
                    //                Day09C: $("#bbit-cal-personc").val(),//参与人工号
                    //                Day10: zone,//时间戳
                    //            }
                    //        }
                ...
                //    var div = document.getElementById("bbit-cal-buddle");
                //    var x = e.clientX;
                //    var y = e.clientY;
            ...
            var caldidian = $("#bbit-cal-didian").val("");
            var calperson = $("#bbit-cal-person").val("");
            var calisedit = $("#bbit-cal-isedit").val("1");
